...
(defun extract-num (str mode)
  ...
  (let ((token (cl-ppcre:scan-to-strings "[0-9a-fA-F]{2,4}" str)))
    (cond ((and token (member mode '(absolute absolute-x absolute-y indirect)))
           (mapcar (lambda (x) (parse-integer x :radix 16))
                   (list (subseq token 2) (subseq token 0 2))))
          (token (parse-integer token :radix 16)))))

(defun asm (source)
  "Assemble SOURCE string into a bytevector and return it."
  (apply 'concatenate 'vector
         (loop for line in (mapcar #'extract-code (cl-ppcre:split "\\n" source))
            for pc = 0 then (+ pc (length (cl-ppcre:split "\\ " line)))
