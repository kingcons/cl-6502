...
(defmacro defopcode (name (&key (docs "") addr-style (track-pc t))
                     modes &body body)
  "Add the opcode variations in MODES to *OPCODES*, then define a function NAME
with DOCS if provided that executes BODY."
  (flet ((gen-accessors (mode)
           (let ((getter (cond ((cl:and (eql addr-style :mixed)
                                        (eql mode 'accumulator)) mode)
                               ((eql addr-style :raw) mode)
                               (t `(lambda (cpu) (get-byte (,mode cpu)))))))
             (list getter `(lambda (x) (setf (,mode cpu) x))))))
    `(progn
       (eval-when (:compile-toplevel :load-toplevel)
         ,@(loop for (op cycles bytes mode) in modes
              collect `(setf (aref *opcodes* ,op) '(,name ,cycles ,bytes
                                                    ,@(gen-accessors mode)))))
       (defun ,name (,(intern "CPU") &optional ,(intern "CYCLES")
                     ,(intern "BYTES") ,(intern "MODE") ,(intern "SETF-FORM"))
         ,docs ,@body
         ,@(when track-pc '((incf (cpu-pc cpu) (1- bytes))))
