...
(defasm adc (:docs "Add to Accumulator with Carry")
    ((#x61 6 2 indirect-x)
     ...
     (#x79 4 3 absolute-y)
     (#x7d 4 3 absolute-x))
  (let ((result (+ (cpu-ar cpu) (getter mode style cpu) (status-bit :carry cpu))))
    (set-flags-if cpu :carry (> result #xff)
                  :overflow (overflow-p result (cpu-ar cpu) (getter mode style cpu))
...
(defasm and (:docs "And with Accumulator")
    ((#x21 6 2 indirect-x)
     ...
     (#x39 4 3 absolute-y)
     (#x3d 4 3 absolute-x))
  (let ((result (setf (cpu-ar cpu) (logand (cpu-ar cpu) (getter mode style cpu)))))
    (set-flags-nz cpu result)))

(defasm asl (:docs "Arithmetic Shift Left" :raw :mixed)
    ((#x06 5 2 zero-page)
     (#x0a 2 1 accumulator)
     (#x0e 6 3 absolute)
     (#x16 6 2 zero-page-x)
     (#x1e 7 3 absolute-x))
  (set-flags-if cpu :carry (logbitp 7 (getter mode style cpu)))
  (let ((result (wrap-byte (ash (getter mode style cpu) 1))))
    (set-flags-nz cpu result)
    (setter mode result cpu)))
...
(defasm bit (:docs "Test Bits in Memory with Accumulator")
    ((#x24 3 2 zero-page)
     (#x2c 4 3 absolute))
  (let ((result (getter mode style cpu)))
...
(defasm cmp (:docs "Compare Memory with Accumulator")
    ((#xc1 6 2 indirect-x)
     ...
     (#xd9 4 3 absolute-y)
     (#xdd 4 3 absolute-x))
  (let ((result (- (cpu-ar cpu) (getter mode style cpu))))
...
(defasm cpx (:docs "Compare Memory with X register")
    ((#xe0 2 2 immediate)
     (#xe4 3 2 zero-page)
     (#xec 4 3 absolute))
  (let ((result (- (cpu-xr cpu) (getter mode style cpu))))
    (set-flags-if cpu :carry (not (minusp result)))
    (set-flags-nz cpu result)))
...
(defasm cpy (:docs "Compare Memory with Y register")
    ((#xc0 2 2 immediate)
     (#xc4 3 2 zero-page)
     (#xcc 4 3 absolute))
  (let ((result (- (cpu-yr cpu) (getter mode style cpu))))
    (set-flags-if cpu :carry (not (minusp result)))
    (set-flags-nz cpu result)))
...
(defasm dec (:docs "Decrement Memory")
    ((#xc6 5 2 zero-page)
     ...
     (#xd6 6 2 zero-page-x)
     (#xde 7 3 absolute-x))
  (let ((result (wrap-byte (1- (getter mode style cpu)))))
    (setter mode result cpu)
    (set-flags-nz cpu result)))
...
(defasm eor (:docs "Exclusive OR with Accumulator")
    ((#x41 6 2 indirect-x)
     ...
     (#x59 4 3 absolute-y)
     (#x5d 4 3 absolute-x))
  (let ((result (setf (cpu-ar cpu) (logxor (getter mode style cpu) (cpu-ar cpu)))))
    (set-flags-nz cpu result)))
...
(defasm jmp (:docs "Jump Unconditionally" :raw t :track-pc nil)
    ((#x4c 3 3 absolute)
     (#x6c 5 3 indirect))
  (setf (cpu-pc cpu) (getter mode style cpu)))
...
(defasm lda (:docs "Load Accumulator from Memory")
    ((#xa1 6 2 indirect-x)
     ...
     (#xb9 4 3 absolute-y)
     (#xbd 4 3 absolute-x))
  (let ((result (setf (cpu-ar cpu) (getter mode style cpu))))
    (set-flags-nz cpu result)))
...
(defasm ldx (:docs "Load X register from Memory")
    ((#xa2 2 2 immediate)
     ...
     (#xb6 4 2 zero-page-y)
     (#xbe 4 3 absolute-y))
  (let ((result (setf (cpu-xr cpu) (getter mode style cpu))))
    (set-flags-nz cpu result)))
...
(defasm ldy (:docs "Load Y register from Memory")
    ((#xa0 2 2 immediate)
     ...
     (#xbc 4 3 absolute-x)
     (#xb4 4 2 zero-page-x))
  (let ((result (setf (cpu-yr cpu) (getter mode style cpu))))
    (set-flags-nz cpu result)))

(defasm lsr (:docs "Logical Shift Right" :raw :mixed)
    ((#x46 5 2 zero-page)
     (#x4a 2 1 accumulator)
     (#x4e 6 3 absolute)
     (#x56 6 2 zero-page-x)
     (#x5e 7 3 absolute-x))
  (set-flags-if cpu :carry (logbitp 0 (getter mode style cpu)))
  (let ((result (ash (getter mode style cpu) -1)))
    (setter mode result cpu)
    (set-flags-nz cpu result)))
...
(defasm ora (:docs "Bitwise OR with Accumulator")
    ((#x01 6 2 indirect-x)
     ...
     (#x19 4 3 absolute-y)
     (#x1d 4 3 absolute-x))
  (let ((result (setf (cpu-ar cpu) (logior (cpu-ar cpu) (getter mode style cpu)))))
    (set-flags-nz cpu result)))
...
(defasm rol (:docs "Rotate Left" :raw :mixed)
    ((#x2a 2 1 accumulator)
     (#x26 5 2 zero-page)
     (#x2e 6 3 absolute)
     (#x36 6 2 zero-page-x)
     (#x3e 7 3 absolute-x))
  (let* ((val (getter mode style cpu))
         (result (wrap-byte (rotate-byte val 1 cpu))))
    (setter mode result cpu)
    (set-flags-if cpu :carry (logbitp 7 val))
    (set-flags-nz cpu result)))

(defasm ror (:docs "Rotate Right" :raw :mixed)
    ((#x66 5 2 zero-page)
     (#x6a 2 1 accumulator)
     (#x6e 6 3 absolute)
     (#x76 6 2 zero-page-x)
...
(defasm sta (:docs "Store Accumulator" :raw t)
    ((#x81 6 2 indirect-x)
     ...
     (#x99 5 3 absolute-y)
     (#x9d 5 3 absolute-x))
  (setter mode (cpu-ar cpu) cpu))

(defasm stx (:docs "Store X register" :raw t)
    ((#x86 3 2 zero-page)
     (#x8e 4 3 absolute)
     (#x96 4 2 zero-page-y))
  (setter mode (cpu-xr cpu) cpu))

(defasm sty (:docs "Store Y register" :raw t)
    ((#x84 3 2 zero-page)
     (#x8c 4 3 absolute)
     (#x94 4 2 zero-page-x))
  (setter mode (cpu-yr cpu) cpu))
