...
(defasm adc (:docs "Add to Accumulator with Carry")
    ((#x61 6 2 indirect-x)
     ...
     (#x79 4 3 absolute-y)
     (#x7d 4 3 absolute-x))
  (let ((result (+ (cpu-ar cpu) (getter mode raw-p cpu) (status-bit :carry cpu))))
    (set-flags-if cpu :carry (> result #xff)
                  :overflow (overflow-p result (cpu-ar cpu) (getter mode raw-p cpu))
...
(defasm and (:docs "And with Accumulator")
    ((#x21 6 2 indirect-x)
     ...
     (#x39 4 3 absolute-y)
     (#x3d 4 3 absolute-x))
  (let ((result (setf (cpu-ar cpu) (logand (cpu-ar cpu) (getter mode raw-p cpu)))))
...
(defasm bit (:docs "Test Bits in Memory with Accumulator")
    ((#x24 3 2 zero-page)
     (#x2c 4 3 absolute))
  (let ((result (getter mode raw-p cpu)))
...
(defasm cmp (:docs "Compare Memory with Accumulator")
    ((#xc1 6 2 indirect-x)
     ...
     (#xd9 4 3 absolute-y)
     (#xdd 4 3 absolute-x))
  (let ((result (- (cpu-ar cpu) (getter mode raw-p cpu))))
...
(defasm cpx (:docs "Compare Memory with X register")
    ((#xe0 2 2 immediate)
     (#xe4 3 2 zero-page)
     (#xec 4 3 absolute))
  (let ((result (- (cpu-xr cpu) (getter mode raw-p cpu))))
    (set-flags-if cpu :carry (not (minusp result)))
    (set-flags-nz cpu result)))
...
(defasm cpy (:docs "Compare Memory with Y register")
    ((#xc0 2 2 immediate)
     (#xc4 3 2 zero-page)
     (#xcc 4 3 absolute))
  (let ((result (- (cpu-yr cpu) (getter mode raw-p cpu))))
    (set-flags-if cpu :carry (not (minusp result)))
    (set-flags-nz cpu result)))
...
(defasm dec (:docs "Decrement Memory")
    ((#xc6 5 2 zero-page)
     ...
     (#xd6 6 2 zero-page-x)
     (#xde 7 3 absolute-x))
  (let ((result (wrap-byte (1- (getter mode raw-p cpu)))))
    (setter mode result cpu)
    (set-flags-nz cpu result)))
...
(defasm eor (:docs "Exclusive OR with Accumulator")
    ((#x41 6 2 indirect-x)
     ...
     (#x59 4 3 absolute-y)
     (#x5d 4 3 absolute-x))
  (let ((result (setf (cpu-ar cpu) (logxor (getter mode raw-p cpu) (cpu-ar cpu)))))
    (set-flags-nz cpu result)))
...
(defasm jmp (:docs "Jump Unconditionally" :raw-p t :track-pc nil)
    ((#x4c 3 3 absolute)
     (#x6c 5 3 indirect))
  (setf (cpu-pc cpu) (getter mode raw-p cpu)))
...
(defasm lda (:docs "Load Accumulator from Memory")
    ((#xa1 6 2 indirect-x)
     ...
     (#xb9 4 3 absolute-y)
     (#xbd 4 3 absolute-x))
  (let ((result (setf (cpu-ar cpu) (getter mode raw-p cpu))))
    (set-flags-nz cpu result)))
...
(defasm ldx (:docs "Load X register from Memory")
    ((#xa2 2 2 immediate)
     ...
     (#xb6 4 2 zero-page-y)
     (#xbe 4 3 absolute-y))
  (let ((result (setf (cpu-xr cpu) (getter mode raw-p cpu))))
    (set-flags-nz cpu result)))
...
(defasm ldy (:docs "Load Y register from Memory")
    ((#xa0 2 2 immediate)
     ...
     (#xbc 4 3 absolute-x)
     (#xb4 4 2 zero-page-x))
  (let ((result (setf (cpu-yr cpu) (getter mode raw-p cpu))))
    (set-flags-nz cpu result)))
...
(defasm ora (:docs "Bitwise OR with Accumulator")
    ((#x01 6 2 indirect-x)
     ...
     (#x19 4 3 absolute-y)
     (#x1d 4 3 absolute-x))
  (let ((result (setf (cpu-ar cpu) (logior (cpu-ar cpu) (getter mode raw-p cpu)))))
    (set-flags-nz cpu result)))
