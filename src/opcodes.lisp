...
(defasm asl (:docs "Arithmetic Shift Left" :raw t)
    ((#x06 5 2 zero-page)
     (#x0a 2 1 accumulator)
     (#x0e 6 3 absolute)
     (#x16 6 2 zero-page-x)
  ...
  (let* ((value (getter-mixed))
    ...
    (set-flags-if cpu :carry (logbitp 7 value))
    (set-flags-nz cpu result)
...
(defasm lsr (:docs "Logical Shift Right" :raw t)
    ((#x46 5 2 zero-page)
     (#x4a 2 1 accumulator)
     (#x4e 6 3 absolute)
     (#x56 6 2 zero-page-x)
  ...
  (let* ((value (getter-mixed))
         (result (ash value -1)))
    (set-flags-if cpu :carry (logbitp 0 value))
    (setter mode result cpu)
    (set-flags-nz cpu result)))
...
(defasm rol (:docs "Rotate Left" :raw t)
    ((#x2a 2 1 accumulator)
     (#x26 5 2 zero-page)
     (#x2e 6 3 absolute)
     (#x36 6 2 zero-page-x)
...
(defasm ror (:docs "Rotate Right" :raw t)
    ((#x66 5 2 zero-page)
     (#x6a 2 1 accumulator)
     (#x6e 6 3 absolute)
     (#x76 6 2 zero-page-x)
